<script>
  import { fetchTokenFreqs } from '../lib/api.js'
  ...
  async function search() {
    ...
    items = []
    try {
      const result = await fetchTokenFreqs(corpusId, { q: shared.query, attr, maxItems, flimit })
...
<SectionCard title="Token Frequencies">
  <form onsubmit={(e) => { e.preventDefault(); search() }}>
    <div class="form-grid">
      <label>
        Query (CQL)
        <input type="text" bind:value={shared.query} placeholder='[pos="NN.*"]' required />
      </label>
      <label>
        Attribute
        <input type="text" bind:value={attr} placeholder="lemma" />
      </label>
      <label>
        Max items
  ...
  {:else if !loading && items.length === 0 && error === null}
    <p class="empty">No results yet. Enter a query and click "Get Frequencies".</p>
  {/if}
</SectionCard>
